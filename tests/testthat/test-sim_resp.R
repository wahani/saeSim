...
test_that("Construction of response works correctly", {
  result1 <- sim_base() %>% sim_gen_fe() %>% sim_resp(resp_eq(y = 100 + x))
  result2 <- sim_base() %>% sim_gen_fe() %>% sim_resp(resp_eq(y = 10 + 0 * x))
  result3 <- sim_base() %>% sim_gen_fe() %>% sim_resp(resp_eq(y = 0 + 0 * x))
