...
#.libPaths("../../../libWin")
library(saeSim)
setup <- sim_setup(sim_base_standard(nDomains = 100, nUnits = 100), R=100, simName="demo", idC=TRUE)
```
...
```{r}
setup <- setup %+% sim_calc(calc_var(varName=c(N = "y"), funNames="length", exclude=NULL), level="population") %+%
  sim_calc(calc_var(c(n = "y"), "length"), level = "sample")
```
